...
export default function TournamentDetailPage() {
  ...
  const handleLogin = () => {
    ...
    addToast({
      ...
      message: 'Vous pouvez maintenant voir les informations du tournoi',
      type: 'success'
    });
  };
  ...
  const handleTicketValidation = () => {
    if (ticketInput.trim().toUpperCase() === 'FF1234-ABCD') {
      ...
      addToast({
        title: 'Ticket validé avec succès !',
        message: 'Vous pouvez maintenant vous inscrire au tournoi',
        type: 'success'
      });
    } else {
      addToast({
  ...
  const copyContact = async () => {
    if (tournament?.organizerContact) {
      try {
        ...
        addToast({
          title: 'Numéro copié !',
          message: 'Le numéro a été copié dans le presse-papiers',
          type: 'success'
        });
      } catch (e) {
        addToast({
  ...
  const handleRegistration = () => {
    ...
    if (tournament?.requiresTicket && !ticketValidated) {
      addToast({
        title: 'Code requis',
        message: 'Ticket d\'accès requis pour ce tournoi privé',
        type: 'error'
      });
      return;
    }
    ...
    router.push(`/tournaments/${params.id}/register`);
  };
        ...
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <button 
              ...
              className="mt-4 px-4 py-2 bg-indigo-600 rounded-md hover:bg-indigo-500 transition"
            >
              Retour aux tournois
            </button>
          </div>
          ...
          <aside className="space-y-6">
            ...
            <div className="rounded-xl border border-neutral-900 bg-neutral-950/60 p-5">
              ...
              <div className="mt-4 grid grid-cols-1 gap-2">
                ...
                >
                  <UserPlus className="w-4 h-4" />
                  {tournament.status === 'open' ? 'S\'inscrire maintenant' : 'Inscriptions fermées'}
                </button>
              </div>
